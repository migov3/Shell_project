		...
		default :             /* some other character */
			...
			if (inputBuffer[i] == '&') // background indicator
			{
				...
				*background=1;
				...
				args[ct] = NULL; /* no more arguments to this command */
				i=length; // make sure the for loop ends now
			}else if (inputBuffer[i] == '>' || inputBuffer[i] == '<' || inputBuffer[i] == '|'){
				if (inputBuffer[i] == '|') *redirec=3;
				else *redirec = (inputBuffer[i] == '>') ? 1 : 2;
				if (start != -1)
				{
					args[ct] = &inputBuffer[start];
					ct++;
				}
				if(*redirec!=3)inputBuffer[i] = '\0';
			}
...
job * new_job(pid_t pid, const char * command, enum job_state state,char *argv[256])
{
  ...
  while(argv[cont]!=0){
      aux->argv[cont] = strdup(argv[cont]);
      cont++;
  }
  aux->argv[cont] = NULL;
	aux->next=NULL;
